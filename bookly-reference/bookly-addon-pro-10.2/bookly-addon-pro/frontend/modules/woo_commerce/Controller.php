...
class Controller extends BooklyLib\Base\Component
{
    ...
    public static function init()
    {
        ...
        add_filter( 'woocommerce_hydration_request_after_callbacks', array( $self, 'hydrationRequestAuto' ), 99, 3 );
    }
    ...
    public static function quantityArgs( $args, $product )
    {
        ...
        if ( $wc_product_ids === null ) {
            ...
            $wc_product_ids = BooklyLib\Proxy\GiftCards::prepareWcProductIds( $wc_product_ids );
        ...
        if ( in_array( $product->get_id(), $wc_product_ids ) ) {
            $args['min_value'] = isset( $args['input_value'] ) ? $args['input_value'] : 1;
        ...
        if ( isset ( $wc_item['bookly'] ) ) {
            ...
            if ( isset ( $wc_item['bookly']['version'] ) && $wc_item['bookly']['version'] == self::VERSION ) {
                ...
                foreach ( $cart_items as $cart_item ) {
                    ...
                    if ( $cart_item->getType() === BooklyLib\CartItem::TYPE_GIFT_CARD ) {
                        $gift_card_type = BooklyLib\Proxy\GiftCards::findGiftCardType( $cart_item->getCartTypeId() );
                    } elseif ( $cart_item->getType() === BooklyLib\CartItem::TYPE_EVENT ) {
                        $product_label = __( 'Event', 'bookly' );
                        $event_ticket_type = BooklyLib\Proxy\Events::getTicketType( $cart_item->getTicketTypeId() );
                        $event = $event_ticket_type ? BooklyLib\Proxy\Events::getEvent( $event_ticket_type->getEventId() ) : null;

                        if ( $event ) {
                            $appointment_start_client_dp = BooklyLib\Slots\DatePoint::fromStr( $event->getStartDate() )->toClientTz();
                            $appointment_end_client_dp = BooklyLib\Slots\DatePoint::fromStr( $event->getEndDate() )->toClientTz();
                        }
                    ...
                    if ( $cart_item->getType() === BooklyLib\CartItem::TYPE_GIFT_CARD ) {
                        ...
                        $codes['event_info'] = $event ? $event->getTranslatedInfo() : '';
                        $codes['event_name'] = $event ? $event->getTranslatedTitle() : __( 'Event was not found', 'bookly' );
                        $codes['ticket_name'] = $event_ticket_type ? $event_ticket_type->getTranslatedTitle() : __( 'Ticket was not found', 'bookly' );
                        $codes['service_price'] = $event_ticket_type ? BooklyLib\Utils\Price::format( $event_ticket_type->getPrice() ) : '';
                        if ( $event && $event->getWcProductId() > 0 ) {
                            if ( ( count( $cart_items ) === 1 ) && $event->getTranslatedWCCartInfoName() ) {
                                $product_label = $event->getTranslatedWCCartInfoName();
                            }
                        }
                        $info_template = '{event_name} ' . __( 'Ticket', 'bookly' ) . ': {ticket_name} &times;{number_of_persons} ' . _x( 'on', 'on date', 'bookly' ) . ' {appointment_date} ' . _x( 'at', 'at time', 'bookly' ) . ' {appointment_time}';
                    } else {
                    ...
                    $info_lines[] = BooklyLib\Utils\Codes::replace( $info_template, $codes, false );
                }
            }

            $other_data[] = array(
                'name' => $product_label,
                'value' => implode( PHP_EOL . PHP_EOL, $info_lines ),
            );
        }
     ...
     */
    public static function hydrationRequestAuto( $response, $handler, $request )
    {
        try {
            $data = $response->get_data();
            // The hydration filter now fires for many stores (products, cart, checkout, ...).
            // Only the checkout/cart response carries a billing_address object — skip the rest.
            if ( ! is_array( $data ) || empty( $data['billing_address'] ) || ! is_object( $data['billing_address'] ) ) {
                return $response;
            }
            foreach ( WC()->cart->get_cart() as $wc_key => $wc_item ) {
                if ( array_key_exists( 'bookly', $wc_item ) ) {
                    foreach ( get_object_vars( $data['billing_address'] ) as $key => $value ) {
                        if ( ! $value && isset( $wc_item['bookly'][ $key ] ) ) {
                            $data['billing_address']->$key = $wc_item['bookly'][ $key ];
                        }
                    }
                    if ( $data['billing_address']->first_name === '' && $data['billing_address']->last_name === '' ) {
                        $data['billing_address']->first_name = $wc_item['bookly']['full_name'];
                    }
                }
            }
        } catch ( \Exception $e ) {
            ...
            if ( $cart_item === null ) {
                ...
                foreach ( $userData->cart->getItems() as $cart_item ) {
                    ...
                    if ( $slots === null || $slots[0][2] === null ) {
                        // If identical items with no date/time (tasks, packages, gift cards) are added to WC cart,
                        // WC will increase quantity for such items instead of creating a new one.
                        // To allow client add more than one such item in cart, generate random line.
    ...
    protected static function getWCProductId( $items )
    {
        ...
        if ( $type === 'gift_card' ) {
            if ( $gift_card_type_id ) {
                $gc_product_id = BooklyLib\Proxy\GiftCards::getWcProductIdByCartTypeId( $item_data['cart_type_id'] );
                if ( $gc_product_id ) {
    ...
    protected static function _getIntersectedItem( array $new_items, &$failed_key )
    {
        ...
        foreach ( $new_items as $key => $candidate_cart_item ) {
            ...
            foreach ( $wc_items as $wc_cart_item ) {
                ...
                if ( $wc_cart_slots[0][2] === null ) {
                    // Item without date can't overlap with other items
                    continue;
                }
