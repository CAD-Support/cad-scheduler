...
class Ajax extends BooklyLib\Base\Ajax
{
    ...
    public static function createPaymentAdjustment()
    {
        ...
        if ( isset( $adjustment['amount'] ) && is_numeric( $adjustment['amount'] ) ) {
            ...
            $payment
                ->setTotal( $payment->getTotal() + $adj_details->getAmount() + ( $data['tax_in_price'] === 'excluded' ? $adj_details->getTax() : 0 ) )
    ...
    public static function updatePaymentAdjustment()
    {
        ...
        if ( isset( $adjustment['amount'] ) && is_numeric( $adjustment['amount'] ) ) {
            ...
            if ( isset( $adjustments[ $index ] ) ) {
                $total = $payment->getTotal() - $adjustments[ $index ]['amount'] - ( $data['tax_in_price'] === 'excluded' ? $adjustments[ $index ]['tax'] : 0 );
                $tax = $payment->getTax() - $adjustments[ $index ]['tax'];
                $adjustments[ $index ] = $adjustment;
                $details->setData( compact( 'adjustments' ) );
                $payment
                    ->setTotal( $total + $adjustment['amount'] + ( $data['tax_in_price'] === 'excluded' ? $adjustment['tax'] : 0 ) )
