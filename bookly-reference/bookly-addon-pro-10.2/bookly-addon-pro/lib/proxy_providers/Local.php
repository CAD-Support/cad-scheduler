...
class Local extends BooklyLib\Proxy\Pro
{
     ...
     * @inheritDoc
     */
    public static function deleteOnlineMeeting( BooklyLib\Base\Entity $entity )
    {
        // Zoom.
        if ( $entity->getOnlineMeetingProvider() == 'zoom' ) {
            if ( ( $entity instanceof Appointment ) || ( $entity instanceof Event ) ) {
                ...
                $zoom = new Zoom\Meetings( Staff::find( $staff_id ) );
                $zoom->remove( $entity->getOnlineMeetingId() );
            }
        }
    }
     ...
     * @inheritDoc
     */
    public static function getLastCustomerLocale( $customer_id )
    {
        return CustomerAppointment::query()
            ->where( 'customer_id', $customer_id )
            ->whereNot( 'locale', null )
            ->sortBy( 'created_at' )
            ->order( 'DESC' )
            ->limit( 1 )
            ->fetchVar( 'locale' );
    ...
    public static function isValidForCartItem( $default, BooklyLib\CartItem $item, BooklyLib\CartInfo $cart_info )
    {
        ...
        if ( $gct === null && $cart_info->getGiftCard() ) {
            $gct = BooklyLib\Proxy\GiftCards::findGiftCardType( $cart_info->getGiftCard()->getGiftCardTypeId() );
        }
        if ( $gct ) {
     ...
     * @inheritDoc
     */
    public static function getMinimumTimePriorBooking( $service_id = 0 )
    {
     ...
     * @inheritDoc
     */
    public static function getMinimumTimePriorCancel( $service_id = 0 )
    {
    ...
    {
        Components::renderLicenseDialog();
    }
     ...
     * @inheritDoc
     */
    public static function getTagsData( $type = Lib\Entities\Tag::TYPE_CUSTOMER )
    {
        return array(
            'list' => Lib\Entities\Tag::query()->select( 'tag, color_id' )->where( 'type', $type )->fetchArray(),
