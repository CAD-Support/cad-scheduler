...
{
    public function update_9_9()
    {
        add_option( 'bookly_cal_frontend_display_only_busy_slots', '0' );
    }

    public function update_9_6()
    {
        add_option( 'bookly_combined_price_method', 'regular' );
    }

    public function update_9_3()
    {
    ...
    public function update_9_1()
    {
        $this->alterTables( array(
            'bookly_tags' => array(
                'ALTER TABLE `%s` CHANGE `type` `type` ENUM("customer","service","event") NOT NULL DEFAULT "customer"',
            ),
            'bookly_forms' => array(
                'ALTER TABLE `%s` CHANGE `type` `type` ENUM("search-form","services-form","staff-form","cancellation-confirmation","tags-form","events-form","checkout-form") NOT NULL DEFAULT "search-form"',
            ),
        ) );
    }

    public function update_9_0()
    {
        $this->alterTables( array(
            'bookly_forms' => array(
                'ALTER TABLE `%s` CHANGE `type` `type` ENUM("search-form","services-form","staff-form","cancellation-confirmation","tags-form","events-form") NOT NULL DEFAULT "search-form"',
            ),
        ) );
    }

    public function update_8_9()
    {
        $this->alterTables( array(
            'bookly_tags' => array(
                'ALTER TABLE `%s` ADD COLUMN `type` ENUM("customer","service") NOT NULL DEFAULT "customer"',
                'ALTER TABLE `%s` ADD COLUMN `info` TEXT DEFAULT NULL',
                'ALTER TABLE `%s` ADD COLUMN `attachment_id` INT UNSIGNED DEFAULT NULL',
            ),
            'bookly_forms' => array(
                'ALTER TABLE `%s` CHANGE `type` `type` ENUM("search-form","services-form","staff-form","cancellation-confirmation","tags-form") NOT NULL DEFAULT "search-form"',
            ),
        ) );
    }
    ...
    public function update_6_3()
    {
        if ( $this->existsColumn( 'bookly_gift_card_type_staff', 'gift_card_id' ) && $this->existsColumn( 'bookly_gift_card_type_staff', 'gift_card_type_id' ) ) {
            $this->dropTableColumns( 'bookly_gift_card_type_staff', array( 'gift_card_id' ) );
        }
    ...
    public function update_6_1()
    {
        ...
        if ( ! $this->existsColumn( 'bookly_gift_card_types', 'title' ) ) {
            ...
            $disposable_options[] = $this->disposable( __FUNCTION__ . '-rename-table', function() use ( $self ) {
                ...
                $self->dropTableForeignKeys( 'bookly_payments', array( 'gift_card_id' ) );
            ...
            $disposable_options[] = $this->disposable( __FUNCTION__ . '-delete-columns', function() use ( $self ) {
                $self->dropTableColumns( 'bookly_gift_card_types', array( 'balance', 'customer_id', 'code' ) );
            } );
