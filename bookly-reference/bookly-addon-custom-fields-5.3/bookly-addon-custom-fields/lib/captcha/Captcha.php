...
class Captcha
{
    ...
    public static function init( $form_id, $config = array() )
    {
        ...
        // Generate CAPTCHA code if not set by a user
        ...
        $form_session = Lib\FormSession::loadSession( $form_id );
        $form_session['captcha'] = $captcha_config;
        Lib\FormSession::saveSession( $form_id, $form_session );
    }
    ...
    public static function draw( $form_id )
    {
        if ( function_exists( 'gd_info' ) ) {
            $form_session = Lib\FormSession::loadSession( $form_id );
            $captcha_config = $form_session['captcha'];
            if ( ! $captcha_config ) {
                return;
            }
    ...
    {
        $form_session = Lib\FormSession::loadSession( $form_id );
        if ( $captcha = $form_session['captcha'] ) {
